...
//#include "../../../modules/task_4/kudryashov_n_gauss_vertical/kudryashov_n_gauss_vertical_std.h"
#include "kudryashov_n_gauss_vertical_std.h"
...
std::vector<std::vector<int>> gauss_filter_parallel(
    const std::vector<std::vector<int>>& image, int height,
    int width, int threadNum) {

    std::vector<std::vector<int>> resultImage(height, std::vector<int>(width));
    std::vector<std::vector<double>> kernel = createKernel();

    int shift = width / threadNum;
    int remain = width % threadNum;

    std::vector<std::thread> threads(threadNum);

    int firstColumn = 0;
    int lastColumn = shift + remain;

    for (int k = 0; k < threadNum; k++) {
        threads[k] = std::thread([&](int firstColumn, int lastColumn) {
